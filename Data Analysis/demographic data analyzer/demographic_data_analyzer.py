...
def calculate_demographic_data(print_data=True):
    ...
    a=df.loc[df['education']=='Bachelors'].size
    b=df.size
    percentage_bachelors = ((a/b)*100).round(1)
    ...
    num_min_workers =df.loc[df['hours-per-week'] == df['hours-per-week'].min()].size
    lazy=df.loc[df['hours-per-week'] == df['hours-per-week'].min()]
    rich_percentage = ((lazy.loc[lazy['salary']=='>50K'].size/num_min_workers)*100).round(1)

    # What country has the highest percentage of people that earn >50K?
    highest = df.loc[df['salary']=='>50K']['native-country'].value_counts()
    counts=df['native-country'].value_counts()
    percentage=(highest/counts)*100
    highest_earning_country=percentage.idxmax()
    highest_earning_country_percentage = percentage[highest_earning_country].round(1)


    # Identify the most popular occupation for those who earn >50K in India.
    a=df.loc[df['native-country']=='India']
    top_IN_occupation = a.loc[a['salary']=='>50K']['occupation'].value_counts().idxmax()
